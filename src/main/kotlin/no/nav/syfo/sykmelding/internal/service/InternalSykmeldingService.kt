...
import no.nav.syfo.sykmelding.internal.db.hentSporsmalOgSvar
...
class InternalSykmeldingService(private val database: DatabaseInterface) {
    fun hentInternalSykmelding(fnr: String): List<InternalSykmeldingDTO> =
            database.getInternalSykmelding(fnr).map {
                val sporsmal = when {
                    it.status.statusEvent == "SENDT" || it.status.statusEvent == "BEKREFTET" -> database.hentSporsmalOgSvar(it.id)
                    else -> emptyList()
                }
